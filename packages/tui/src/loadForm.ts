...
export const LOAD_FIELD_DEFS: LoadFieldDef[] = [
  ...
  },
  {
    ...
    kind: "enum",
    label: "Split Mode",
    help: "How tensors are split (--split-mode). Layer shares the model across cards. Row needs a fast interconnect. Tensor splits every weight matrix (experimental). None keeps every GPU layer on the Main GPU.",
    store: "load",
    key: "splitMode",
      ...
      name:
        m === "layer"
          ...
          : m === "row"
            ...
            : m === "tensor"
              ? "tensor — splits weight matrices (experimental)"
  ...
  },
  {
    ...
    help: "MTP uses next-n layers in the main GGUF, or a sibling mtp-*.gguf (Gemma 4). DFlash needs a separate draft GGUF. N-gram drafts from the prompt itself and can stack with MTP or DFlash.",
    store: "load",
    key: "speculativeMode",
    options: [
      { value: "off", name: "Off" },
      ...
      { value: "ngram", name: "N-gram (no draft model)" },
      { value: "ngram-mtp", name: "N-gram + MTP" },
      { value: "ngram-dflash", name: "N-gram + DFlash" },
    ],
  },
  {
    id: "ngramVariant",
    kind: "enum",
    label: "N-gram Variant",
    help: "N-gram lookup strategy (--spec-type ngram-*). Simple = rolling prompt window; map-k = indexed lookup for long prompts; mod = classic prompt-lookup.",
    store: "load",
    key: "ngramVariant",
    options: [
      { value: "simple", name: "simple — rolling prompt window" },
      { value: "map-k", name: "map-k — indexed lookup (long prompts)" },
      { value: "map-k4v", name: "map-k4v — indexed + KV cache" },
      { value: "mod", name: "mod — classic prompt-lookup" },
    ],
  },
  {
    id: "ngramSizeN",
    kind: "number",
    label: "N-gram Lookup Size",
    help: "Lookup n-gram length in tokens (--spec-ngram-*-size-n; default 12). Longer = fewer false matches.",
    step: 1,
    store: "load",
    key: "ngramSizeN",
    min: 2,
  ...
  },
  {
    id: "ngramSizeM",
    kind: "number",
    label: "N-gram Draft Length",
    help: "Drafted continuation length in tokens (--spec-ngram-*-size-m; default 48). Caps acceptance per step.",
    step: 1,
    store: "load",
    key: "ngramSizeM",
    min: 2,
  ...
  },
  {
    ...
    help: "Minimum occurrences before drafting (--spec-ngram-*-min-hits; default 1). Raise to 2–3 to skip coincidental matches.",
    step: 1,
    store: "load",
    key: "ngramMinHits",
    min: 1,
  ...
  },
  {
    ...
    help: "MTP/DFlash tokens to draft per step (--spec-draft-n-max). Separate from n-gram draft length. DFlash often uses 8–15.",
    step: 1,
    store: "load",
    key: "maxDraftTokens",
    min: 0,
  ...
  },
  {
    ...
    kind: "number",
    label: "Min P",
    help: "Min-p sampling (request + server CLI default; 0 = disabled). llama-server's built-in 0.05 is wrong for most coder models.",
    step: 0.01,
    store: "request",
    key: "minP",
    min: 0,
  ...
  },
  {
    ...
    help: "Repetition penalty (1.0 = disabled). Above 1 helps prose, harms code.",
    step: 0.01,
    store: "request",
    key: "repeatPenalty",
    min: 0.5,
  ...
  },
  {
    ...
    help: "OpenAI-style presence penalty (0 = disabled). Qwen3 No-Think recommends 1.5.",
    step: 0.05,
    store: "request",
    key: "presencePenalty",
    min: -2,
  ...
  },
  {
    ...
    help: "OpenAI-style frequency penalty (0 = disabled). Scales with token count.",
    step: 0.05,
    store: "request",
