...
export function getDataRoot(config?: vscode.WorkspaceConfiguration): string {
  return path.join(os.homedir(), ".llama-aio-vs");
}

/** Root folder for llama.cpp installs (contains per-backend subdirs). */
export function getInstallDir(config: vscode.WorkspaceConfiguration): string {
...
export function getModelsDir(config: vscode.WorkspaceConfiguration): string {
